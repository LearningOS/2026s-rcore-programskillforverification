...
pub fn sys_mutex_create(blocking: bool) -> isize {
    ...
    process_inner.register_mutex_resource(id);
    id as isize
}
/// mutex lock syscall
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    ...
    if process_inner.deadlock_detect {
        process_inner.ensure_mutex_thread(tid);
        process_inner.mutex_need[tid][mutex_id] += 1;
        if !process_inner.mutex_state_safe() {
            process_inner.mutex_need[tid][mutex_id] -= 1;
            return -0xDEAD;
        }
    }
...
pub fn sys_semaphore_create(res_count: usize) -> isize {
    ...
    process_inner.register_semaphore_resource(id, res_count);
    id as isize
}
...
pub fn sys_semaphore_up(sem_id: usize) -> isize {
    ...
    let mut process_inner = process.inner_exclusive_access();
    if process_inner.deadlock_detect {
        process_inner.ensure_semaphore_thread(tid);
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    ...
    if process_inner.deadlock_detect {
        ...
        if !process_inner.semaphore_state_safe() {
            process_inner.semaphore_need[tid][sem_id] -= 1;
            return -0xDEAD;
        }
    }
